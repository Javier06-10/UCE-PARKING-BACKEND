...
export async function createVehicleHandler(req, res) {
  try {
    const { placa, id_marca, id_modelo, id_color, Marca, Color, modelo, persona_id } = req.body;
    const data = await createVehicle({ placa, id_marca, id_modelo, id_color, Marca, Color, modelo, persona_id });
...
export async function updateVehicleHandler(req, res) {
  try {
    const { placa, id_marca, id_modelo, id_color, Marca, Color, modelo, persona_id } = req.body;
    const data = await updateVehicle(req.params.id, { placa, id_marca, id_modelo, id_color, Marca, Color, modelo, persona_id });
    res.json({ ok: true, data });
  } catch (error) {
